...
import com.itszuvalex.femtocraft.industry.tiles.TileEntityFemtoChronoshifter._

object TileEntityFemtoChronoshifter {
  val outputSlot               = 7
  val inventorySize            = 8
...
@Configurable class TileEntityFemtoChronoshifter extends TileEntityBaseEntityNanoHorologe {
  inventory.setInventorySize(inventorySize)

  override def defaultContainer = new PowerContainer(TECH_LEVEL, POWER_STORAGE)

  override def getGuiID = FemtocraftGuiConstants.FemtoChronoshifterGuiID
  ...
  protected override def getTechLevel = EnumTechLevel.FEMTO
}
