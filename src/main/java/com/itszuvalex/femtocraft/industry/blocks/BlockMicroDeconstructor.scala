...
class BlockMicroDeconstructor extends TileContainer(Material.iron) with DroppableInventory with RotateOnPlace {
  /**
   ...
   */
  override def createNewTileEntity(par1World: World, metadata: Int) = new TileEntityBaseEntityMicroDeconstructor
  ...
  override def breakBlock(world: World, x: Int, y: Int, z: Int, block: Block, metadata: Int) {
    world.getTileEntity(x, y, z) match {
      case te: TileEntityBaseEntityMicroDeconstructor if te.isWorking => FemtocraftUtils.dropItem(te.deconstructingStack, world, x, y, z, rand)
      case _                                                          =>
    }
