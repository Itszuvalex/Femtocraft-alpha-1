...
class BlockNanoDismantler extends TileContainer(Material.iron) with DroppableInventory with RotateOnPlace {
  /**
  ...
  setBlockName("BlockNanoDismantler")
  setHardness(3.5f)
  ...
  @SideOnly(Side.CLIENT) override def registerBlockIcons(par1IconRegister: IIconRegister) {
    ...
    frontIcon = par1IconRegister.registerIcon(Femtocraft.ID.toLowerCase + ":" + "NanoDismantler_front")
  }

  /**
   * Returns a new instance of a block's tile entity class. Called on placing the block.
   */
  override def createNewTileEntity(par1World: World, metadata: Int) = new TileEntityNanoDismantler
  ...
  override def breakBlock(world: World, x: Int, y: Int, z: Int, block: Block, metadata: Int) {
    world.getTileEntity(x, y, z) match {
      case te: TileEntityNanoDismantler if te.isWorking => FemtocraftUtils.dropItem(te.deconstructingStack, world, x, y, z, rand)
      case _                                            =>
    }
