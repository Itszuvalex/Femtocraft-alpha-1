...
class BlockFemtoChronoshifter extends TileContainer(Material.iron) with DroppableInventory with RotateOnPlace {
  /**
  ...
  @SideOnly(Side.CLIENT) override def registerBlockIcons(par1IconRegister: IIconRegister) {
    ...
    frontIcon = par1IconRegister.registerIcon(Femtocraft.ID.toLowerCase + ":" + "FemtoChronoshifter_front")
  }
   ...
   */
  override def createNewTileEntity(par1World: World, metadata: Int) = new TileEntityFemtoChronoshifter
  ...
  override def breakBlock(world: World, x: Int, y: Int, z: Int, block: Block, metadata: Int) {
    world.getTileEntity(x, y, z) match {
      case te: TileEntityFemtoChronoshifter if te.isWorking =>
        FemtocraftUtils.dropItem(te.chronoStack, world, x, y, z, rand)
      case _                                                =>
    }
