...
import com.itszuvalex.femtocraft.core.traits.{DroppableInventory, RotateOnPlace}
...
object BlockFemtoImpulser {
  ...
  def updateFurnaceBlockState(isActive: Boolean, world: World, x: Int, y: Int, z: Int) {
    val l = world.getBlockMetadata(x, y, z)
    val tileentity = world.getTileEntity(x, y, z)
    keepFurnaceInventory = true
    TileContainer.shouldDrop = false
    world.setBlock(x, y, z, if (isActive) Femtocraft.blockFemtoImpulserLit else Femtocraft.blockFemtoImpulserUnlit)
...
class BlockFemtoImpulser(private val isActive: Boolean) extends TileContainer(Material.iron) with DroppableInventory with RotateOnPlace {
  /**
  ...
  if (isActive) setLightLevel(0.875F)

  override def renderAsNormalBlock = false

  override def getRenderType: Int = RenderSimpleMachine.renderID
  ...
  @SideOnly(Side.CLIENT) override def randomDisplayTick(par1World: World, par2: Int, par3: Int, par4: Int, par5Random: Random) {
    if (isActive) {
      val l = par1World.getBlockMetadata(par2, par3, par4)
      val f = par2.toFloat + 0.5F
      val f1 = par3.toFloat + 0.0F + par5Random.nextFloat * 6.0F / 16.0F
      val f2 = par4.toFloat + 0.5F
      val f3 = 0.52F
      val f4 = par5Random.nextFloat * 0.6F - 0.3F
      if (l == 4) {
  ...
  override def getItemDropped(p_149650_1_ : Int, p_149650_2_ : Random, p_149650_3_ : Int) = Item.getItemFromBlock(Femtocraft.blockFemtoImpulserUnlit)

  override def getPickBlock(target: MovingObjectPosition, world: World, x: Int, y: Int, z: Int) = new ItemStack(Femtocraft.blockFemtoImpulserUnlit)
  ...
  @SideOnly(Side.CLIENT) override def registerBlockIcons(par1IconRegister: IIconRegister) {
    ...
    frontIcon = par1IconRegister.registerIcon(if (this.isActive) Femtocraft.ID.toLowerCase + ":" + "FemtoImpulser_front_lit" else Femtocraft.ID.toLowerCase + ":" + "FemtoImpulser_front_unlit")
  }

  /**
   * Returns a new instance of a block's tile entity class. Called on placing the block.
   */
  override def createNewTileEntity(par1World: World, metadata: Int) = new TileEntityFemtoImpulser
