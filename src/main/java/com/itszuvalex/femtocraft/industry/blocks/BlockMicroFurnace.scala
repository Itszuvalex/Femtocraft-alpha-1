...
object BlockMicroFurnace {
  /**
   * This flag is used to prevent the furnace inventory to be dropped upon block removal, is used internally when the
   * furnace block changes from idle to active and vice-versa.
   */
  private var keepFurnaceInventory = false
  ...
  def updateFurnaceBlockState(active: Boolean, world: World, x: Int, y: Int, z: Int) {
    ...
    world.setBlock(x, y, z, if (active) Femtocraft.blockMicroFurnaceLit else Femtocraft.blockMicroFurnaceUnlit)
...
class BlockMicroFurnace(private val isActive: Boolean) extends TileContainer(Material.iron) with DroppableInventory with RotateOnPlace {
  /**
  ...
  override def getPickBlock(target: MovingObjectPosition, world: World, x: Int, y: Int, z: Int) = new ItemStack(Femtocraft.blockMicroFurnaceUnlit)

  override def getItemDropped(p_149650_1_ : Int, p_149650_2_ : Random, p_149650_3_ : Int) = Item.getItemFromBlock(Femtocraft.blockMicroFurnaceUnlit)
  ...
  @SideOnly(Side.CLIENT) override def registerBlockIcons(par1IconRegister: IIconRegister) {
    ...
    frontIcon = par1IconRegister.registerIcon(if (this.isActive) Femtocraft.ID.toLowerCase + ":" + "MicroFurnace_front_lit" else Femtocraft.ID.toLowerCase + ":" + "MicroFurnace_front_unlit")
  }

  /**
   * Returns a new instance of a block's tile entity class. Called on placing the block.
   */
  override def createNewTileEntity(par1World: World, metadata: Int) = new TileEntityBaseEntityMicroFurnace
