...
class BlockFemtoCoagulator extends TileContainer(Material.iron) with DroppableInventory with RotateOnPlace {
  /**
  ...
  setBlockName("BlockFemtoCoagulator")
  setHardness(3.5f)
  ...
  @SideOnly(Side.CLIENT) override def registerBlockIcons(par1IconRegister: IIconRegister) {
    ...
    frontIcon = par1IconRegister.registerIcon(Femtocraft.ID.toLowerCase + ":" + "FemtoCoagulator_front")
  }
   ...
   */
  override def createNewTileEntity(par1World: World, metadata: Int) = new TileEntityFemtoCoagulator
  ...
  override def breakBlock(world: World, x: Int, y: Int, z: Int, block: Block, metadata: Int) {
    world.getTileEntity(x, y, z) match {
      case te: TileEntityFemtoCoagulator if te.isWorking =>
        te.reconstructingStacks.foreach(FemtocraftUtils.dropItem(_, world, x, y, z, rand))
      case _                                             =>
    }
