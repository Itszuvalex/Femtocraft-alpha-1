...
class BlockNanoEnmesher extends TileContainer(Material.iron) with DroppableInventory with RotateOnPlace {
  /**
  ...
  setBlockName("BlockNanoEnmesher")
  setHardness(3.5f)
  ...
  @SideOnly(Side.CLIENT) override def registerBlockIcons(par1IconRegister: IIconRegister) {
    ...
    frontIcon = par1IconRegister.registerIcon(Femtocraft.ID.toLowerCase + ":" + "NanoEnmesher_front")
  }

  /**
   * Returns a new instance of a block's tile entity class. Called on placing the block.
   */
  override def createNewTileEntity(par1World: World, metadata: Int) = new TileEntityBaseEntityNanoEnmesher
  ...
  override def breakBlock(world: World, x: Int, y: Int, z: Int, block: Block, metadata: Int) {
    world.getTileEntity(x, y, z) match {
      case te: TileEntityBaseEntityNanoEnmesher if te.isWorking => FemtocraftUtils.dropItem(te.meshStack, world, x, y, z, rand)
      case _                                                    =>
    }
