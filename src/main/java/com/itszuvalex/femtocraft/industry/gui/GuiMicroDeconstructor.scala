...
@SideOnly(Side.CLIENT) object GuiMicroDeconstructor {
  val texture = new ResourceLocation(Femtocraft.ID.toLowerCase, "textures/guis/Disassembler.png")
}

@SideOnly(Side.CLIENT) class GuiMicroDeconstructor(player: EntityPlayer, par1InventoryPlayer: InventoryPlayer, tileEntity: TileEntityBaseEntityMicroDeconstructor) extends GuiBase(new ContainerMicroDeconstructor(player, par1InventoryPlayer, tileEntity)) {
  private val deconstructorInventory = tileEntity

  override def drawScreen(par1: Int, par2: Int, par3: Float) {
    super.drawScreen(par1, par2, par3)
    if (isPointInRegion(10, 8, 16, 60, par1, par2)) {
      val furnaceCurrent = deconstructorInventory.currentPower
      val furnaceMax = deconstructorInventory.getMaxPower
      ...
      val massCurrent = deconstructorInventory.getMassAmount
      val massMax = deconstructorInventory.getMassCapacity
      val fluid = deconstructorInventory.getTankInfo(ForgeDirection.UNKNOWN)(0).fluid
  ...
  protected def drawGuiContainerBackgroundLayer(par1: Float, par2: Int, par3: Int) {
    ...
    Minecraft.getMinecraft.getTextureManager.bindTexture(GuiMicroDeconstructor.texture)
    val k = (width - xSize) / 2
    val l = (height - ySize) / 2
    drawTexturedModalRect(k, l, 0, 0, xSize, ySize)
    var i1 = 0
    i1 = deconstructorInventory.getCookProgressScaled(26)
    drawTexturedModalRect(k + 60, l + 24, 176, 0, i1 + 1, 40)
    i1 = (deconstructorInventory.currentPower * 60) / deconstructorInventory.getMaxPower
    drawTexturedModalRect(k + 10, l + 8 + (60 - i1), 176, 40 + (60 - i1), 16, i1)
    val fluid = deconstructorInventory.getTankInfo(ForgeDirection.UNKNOWN)(0).fluid
    if (fluid != null) {
      val image = fluid.getFluid.getStillIcon
      i1 = (deconstructorInventory.getMassAmount * 60) / deconstructorInventory.getMassCapacity
