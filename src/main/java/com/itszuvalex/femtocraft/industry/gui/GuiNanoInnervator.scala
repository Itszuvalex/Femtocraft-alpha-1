...
import com.itszuvalex.femtocraft.industry.containers.ContainerNanoInnervator
...
@SideOnly(Side.CLIENT) object GuiNanoInnervator {
  val texture = new ResourceLocation(Femtocraft.ID.toLowerCase, "textures/guis/NanoInnervator.png")
}

@SideOnly(Side.CLIENT) class GuiNanoInnervator(player: EntityPlayer, par1InventoryPlayer: InventoryPlayer, par2TileEntityFurnace: TileEntityNanoInnervator) extends GuiBase(new ContainerNanoInnervator(player, par1InventoryPlayer, par2TileEntityFurnace)) {
  ...
  protected def drawGuiContainerBackgroundLayer(par1: Float, par2: Int, par3: Int) {
    ...
    Minecraft.getMinecraft.getTextureManager.bindTexture(GuiNanoInnervator.texture)
    val k = (width - xSize) / 2
