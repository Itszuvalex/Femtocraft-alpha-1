...
class BlockPhlegethonTunnelCore extends TileContainer(Material.iron) {
  ...
  override def createTileEntity(world: World, metadata: Int) = new TileEntityPhlegethonTunnelCore

  @SideOnly(Side.CLIENT) override def registerBlockIcons(par1IconRegister: IIconRegister) {
    blockIcon = par1IconRegister.registerIcon(Femtocraft.ID.toLowerCase + ":" + "BlockPhlegethonTunnelCore")
  }
