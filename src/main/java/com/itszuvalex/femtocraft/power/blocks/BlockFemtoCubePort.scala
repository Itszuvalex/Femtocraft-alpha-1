...
class BlockFemtoCubePort extends TileContainer(Material.iron) {
  ...
  setBlockName("BlockFemtoCubePort")
  ...
  override def randomDisplayTick(par1World: World, x: Int, y: Int, z: Int, par5Random: Random) {
    par1World.getTileEntity(x, y, z) match {
      case port: TileEntityFemtoCubePort if port.isValidMultiBlock && port.output =>
  ...
  @SideOnly(Side.CLIENT) override def registerBlockIcons(par1IconRegister: IIconRegister) {
    this.blockIcon = {portInput = par1IconRegister.registerIcon(Femtocraft.ID.toLowerCase + ":" + "BlockFemtoCubePort_input"); portInput}
    portOutput = par1IconRegister.registerIcon(Femtocraft.ID.toLowerCase + ":" + "BlockFemtoCubePort_output")
  }

  override def createNewTileEntity(world: World, metadata: Int) = new TileEntityFemtoCubePort
