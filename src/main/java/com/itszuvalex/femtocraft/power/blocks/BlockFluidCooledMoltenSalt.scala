...
class BlockFluidCooledMoltenSalt extends BlockFluidClassic(Femtocraft.fluidCooledMoltenSalt, Material.water) {
  ...
  @SideOnly(Side.CLIENT) override def registerBlockIcons(par1IconRegister: IIconRegister) {
    blockIcon = {stillIcon = par1IconRegister.registerIcon(Femtocraft.ID.toLowerCase + ":" + "BlockCooledMoltenSalt_still"); stillIcon}
    flowingIcon = par1IconRegister.registerIcon(Femtocraft.ID.toLowerCase + ":" + "BlockCooledMoltenSalt_flow")
  }
}
