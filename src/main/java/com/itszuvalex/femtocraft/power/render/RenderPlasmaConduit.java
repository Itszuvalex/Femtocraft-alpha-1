...
//            connector_out_west_filled, connector_out_up_filled, connector_out_down_filled;
    ...
    private boolean renderConduit(BlockPlasmaConduit conduit, int x, int y, int z, boolean filled,
        ...
        if (input != null) {
            switch (input) {
                case DOWN:
                    input_model = connector_in_down;
                    break;
                case UP:
                    input_model = connector_in_up;
                    break;
                case NORTH:
                    input_model = connector_in_north;
                    break;
                case SOUTH:
                    input_model = connector_in_south;
                    break;
                case WEST:
                    input_model = connector_in_west;
                    break;
                case EAST:
                    input_model = connector_in_east;
                    break;
            }
        }
        ...
        if (output != null) {
            switch (output) {
                case DOWN:
                    output_model = connector_out_down;
                    break;
                case UP:
                    output_model = connector_out_up;
                    break;
                case NORTH:
                    output_model = connector_out_north;
                    break;
                case SOUTH:
                    output_model = connector_out_south;
                    break;
                case WEST:
                    output_model = connector_out_west;
                    break;
                case EAST:
                    output_model = connector_out_east;
                    break;
            }
        }

        core_model.location = new RenderPoint(x, y, z);
        IIcon center = !filled ? conduit.center : conduit.center_filled;
        for (RenderQuad quad : core_model.faces) {
            quad.icon = center;
            quad.minU = center.getInterpolatedU(2f);
            quad.maxU = center.getInterpolatedU(14f);
            quad.minV = center.getInterpolatedV(2f);
            quad.maxV = center.getInterpolatedV(14f);
        }
        core_model.draw();
        if (input_model != null) {
            input_model.location = new RenderPoint(x, y, z);
            IIcon inputIcon = !filled ? conduit.input : conduit.input_filled;
            for (RenderQuad quad : input_model.faces) {
                quad.icon = inputIcon;
                quad.minU = inputIcon.getInterpolatedU(0f);
                quad.maxU = inputIcon.getInterpolatedU(2f);
                quad.minV = inputIcon.getInterpolatedV(2f);
                quad.maxV = inputIcon.getInterpolatedV(14f);
            }
            input_model.draw();
        }
        if (output_model != null) {
            output_model.location = new RenderPoint(x, y, z);
            IIcon outputIcon = !filled ? conduit.output : conduit.output_filled;
            for (RenderQuad quad : output_model.faces) {
                quad.icon = outputIcon;
                quad.minU = outputIcon.getInterpolatedU(0f);
