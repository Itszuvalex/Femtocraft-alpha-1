...
import com.itszuvalex.femtocraft.industry.containers.ContainerFemtoEntangler
import com.itszuvalex.femtocraft.industry.gui.GuiFemtoEntangler._
...
@SideOnly(Side.CLIENT) object GuiFemtoEntangler {
  val texture = new ResourceLocation(Femtocraft.ID.toLowerCase, "textures/guis/FemtoEntangler.png")
}

@SideOnly(Side.CLIENT) class GuiFemtoEntangler(player: EntityPlayer, par1InventoryPlayer: InventoryPlayer, entangler: TileEntityFemtoEntangler) extends GuiBase(new ContainerFemtoEntangler(player, par1InventoryPlayer, entangler)) {
  private val inventory = entangler
