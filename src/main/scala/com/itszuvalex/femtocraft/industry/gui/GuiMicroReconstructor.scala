...
import com.itszuvalex.femtocraft.industry.gui.GuiMicroReconstructor._
...
@SideOnly(Side.CLIENT) object GuiMicroReconstructor {
  val texture = new ResourceLocation(Femtocraft.ID.toLowerCase, "textures/guis/Reassembler.png")
}

@SideOnly(Side.CLIENT) class GuiMicroReconstructor(player: EntityPlayer, par1InventoryPlayer: InventoryPlayer, tileEntity: TileEntityBaseEntityMicroReconstructor) extends GuiBase(new ContainerMicroReconstructor(player, par1InventoryPlayer, tileEntity)) {
  private val reconstructorInventory = tileEntity
  ySize = 204

  override def drawScreen(par1: Int, par2: Int, par3: Float) {
    super.drawScreen(par1, par2, par3)
    if (isPointInRegion(10, 8, 16, 60, par1, par2)) {
      val furnaceCurrent = reconstructorInventory.currentPower
      val furnaceMax = reconstructorInventory.getMaxPower
      ...
      val massCurrent = reconstructorInventory.getMassAmount
      val massMax = reconstructorInventory.getMassCapacity
      val fluid = reconstructorInventory.getTankInfo(ForgeDirection.UNKNOWN)(0).fluid
  ...
  protected def drawGuiContainerBackgroundLayer(par1: Float, par2: Int, par3: Int) {
    ...
    var i1 = 0
    i1 = reconstructorInventory.getCookProgressScaled(30)
    drawTexturedModalRect(k + 86, l + 21, 176, 0, i1 + 1, 31)
    i1 = (reconstructorInventory.currentPower * 60) / reconstructorInventory.getMaxPower
    drawTexturedModalRect(k + 10, l + 8 + (60 - i1), 176, 31 + (60 - i1), 16, i1)
    val fluid = reconstructorInventory.getTankInfo(ForgeDirection.UNKNOWN)(0).fluid
    if (fluid != null) {
      val image = fluid.getFluid.getStillIcon
      i1 = (reconstructorInventory.getMassAmount * 60) / reconstructorInventory.getMassCapacity
