...
class ProxyGuiClient extends ProxyGuiCommon {
  override def getClientGuiElement(ID: Int, player: EntityPlayer, world: World, x: Int, y: Int, z: Int): AnyRef = {
    (ID, world.getTileEntity(x, y, z)) match {
      case (FemtocraftGuiConstants.ResearchComputerGuiID, _) => new GuiResearch(player.getUniqueID)
      ...
      case (FemtocraftGuiConstants.ModulationLensMountGuiID, te: TileEntityModulationLensMount) => new GuiModulationLensMount(player, player.inventory, te)
      case (_, _) => null
    }
  }
}
