...
class FemtocraftPlayerTracker {
  ...
  @SubscribeEvent
  def onPlayerLogin(event: PlayerLoggedInEvent) = Femtocraft.researchManager.addPlayerResearch(event.player.getCommandSenderName).sync(event.player.asInstanceOf[EntityPlayerMP])
