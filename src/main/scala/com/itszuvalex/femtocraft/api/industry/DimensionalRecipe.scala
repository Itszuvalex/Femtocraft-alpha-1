 ...
 */
class DimensionalRecipe(@Saveable
                        @Configurable(comment = "ItemStack that is consumed to use this recipe.")
                        var input: ItemStack, @Saveable
                        @Configurable(comment = "Null, ItemStack[4] or ItemStack[12] (can include nulls) that dictate configuration items" + " required to induce creation. Note, ItemStack[4] is associated with Nano level Enmesher," + " ItemStack[12] is associated with Femto.")
                        ...
                        var tech: String) extends Comparable[DimensionalRecipe] with ISaveable {
  ...
  def compareTo(tr: DimensionalRecipe): Int = {
