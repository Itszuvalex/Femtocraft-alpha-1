 ...
 */
class CommandResearchDiscover extends CommandBase("discover", null) {
  ...
  override def processCommand(iCommandSender: ICommandSender, astring: Array[String]): Unit = {
    iCommandSender match {
      case player: EntityPlayerMP if MinecraftServer.getServer != null && MinecraftServer.getServer.getConfigurationManager.func_152596_g(player.getGameProfile) =>
        ...
        astring(1) match {
          case "all" =>
            Femtocraft.researchManager.getTechnologies.filterNot(research.hasDiscoveredTechnology).map(_.getName).foreach(research.discoverTechnology)
            FemtocraftUtils.sendMessageToPlayer(player, "Player: " + astring(0) + " just had all techs discovered.")
          case tech =>
            research.discoverTechnology(astring(1).replaceAll("_", "\\ "), true)
            FemtocraftUtils.sendMessageToPlayer(player, "Player: " + astring(0) + " just had " + astring(1).replaceAll("_", "\\ ") + " discovered.")
        }
    ...
    icommandsender match {
      case player: EntityPlayerMP if MinecraftServer.getServer != null && MinecraftServer.getServer.getConfigurationManager.func_152596_g(player.getGameProfile) =>
        if (astring.length == 1)
          ...
          Femtocraft.researchManager.getTechnologies.filterNot(research.hasDiscoveredTechnology)
