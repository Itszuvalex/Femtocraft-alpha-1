...
object GuiFemtoCube {
  val texture = new ResourceLocation(Femtocraft.ID.toLowerCase, "textures/guis/FemtoCube.png")
}

class GuiFemtoCube(private val controller: TileEntityFemtoCubePort) extends GuiBase(new ContainerFemtoCube(controller)) {
  ...
  protected def drawGuiContainerBackgroundLayer(f: Float, i: Int, j: Int) {
    ...
    Minecraft.getMinecraft.getTextureManager.bindTexture(GuiFemtoCube.texture)
