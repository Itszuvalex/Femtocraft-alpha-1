...
class BlockFemtoCubeChassis extends TileContainer(Material.iron) with MultiBlockSpatialReactions {
  ...
  @SideOnly(Side.CLIENT) override def getIcon(par1iBlockAccess: IBlockAccess, par2: Int, par3: Int, par4: Int, par5: Int): IIcon = par1iBlockAccess.getTileEntity(par2, par3, par4) match {
    case chassis: TileEntityFemtoCubeChassis if chassis.isValidMultiBlock =>
  ...
  @SideOnly(Side.CLIENT) override def registerBlockIcons(par1IconRegister: IIconRegister) {
    blockIcon = par1IconRegister.registerIcon(Femtocraft.ID.toLowerCase + ":" + "BlockFemtoCubeChassis_unformed")
    for (x <- 0 until icons.length) {
      for (y <- 0 until icons(x).length) {
        if (!(x > 0 && x < icons.length - 1 && y > 0) || !(y < icons(x).length - 1)) {
          icons(x)(y) = par1IconRegister.registerIcon(Femtocraft.ID.toLowerCase + ":" + "BlockFemtoCubeChassis_" + x + "_" + y)
        }
      }
    }
  }

  override def createNewTileEntity(world: World, metadata: Int) = new TileEntityFemtoCubeChassis
