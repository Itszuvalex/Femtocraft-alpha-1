...
import femtocraft.industry.tiles.TileEntityNanoFabricator;
...
public class ContainerNanoFabricator extends Container {
    private TileEntityNanoFabricator fabricator;
    private int lastCookTime = 0;
    private int lastPower = 0;
    private int lastMass = 0;

    public ContainerNanoFabricator(InventoryPlayer par1InventoryPlayer,
                                       TileEntityNanoFabricator fabricator) {
        this.fabricator = fabricator;
        this.addSlotToContainer(new OutputSlot(fabricator, 9, 122, 18));
        Slot schematic = new Slot(fabricator, 10, 93, 54) {
        ...
        for (int y = 0; y < 3; ++y) {
            for (int x = 0; x < 3; ++x) {
                this.addSlotToContainer(new DisplaySlot(fabricator, x + y
                        * 3, 32 + x * 18, 18 + y * 18) {
                    @Override
        ...
        for (int y = 0; y < 2; ++y) {
            for (int x = 0; x < 9; ++x) {
                this.addSlotToContainer(new Slot(fabricator, 11 + x + y * 9,
                                                 8 + x * 18, 77 + y * 18));
            }
        }
    ...
    public void addCraftingToCrafters(ICrafting par1ICrafting) {
        ...
        par1ICrafting.sendProgressBarUpdate(this, 0,
                                            this.fabricator.cookTime);
        par1ICrafting.sendProgressBarUpdate(this, 1,
                                            this.fabricator.getCurrentPower());
        par1ICrafting.sendProgressBarUpdate(this, 2,
                                            this.fabricator.getMassAmount());
    }
    ...
    public void detectAndSendChanges() {
        ...
        for (Object crafter : this.crafters) {
            ...
            if (this.lastCookTime != this.fabricator.cookTime) {
                icrafting.sendProgressBarUpdate(this, 0,
                                                this.fabricator.cookTime);
            }
            if (this.lastPower != this.fabricator.getCurrentPower()) {
                icrafting.sendProgressBarUpdate(this, 1,
                                                this.fabricator.getCurrentPower());
            }
            if (this.lastMass != this.fabricator.getMassAmount()) {
                icrafting.sendProgressBarUpdate(this, 2,
                                                this.fabricator.getMassAmount());
            }
        }

        this.lastCookTime = this.fabricator.cookTime;
        this.lastPower = this.fabricator.getCurrentPower();
        this.lastMass = this.fabricator.getMassAmount();
    }
    ...
    public void updateProgressBar(int par1, int par2) {
        switch (par1) {
            case 0:
                this.fabricator.cookTime = par2;
                break;
            case 1:
                this.fabricator.currentPower = par2;
                break;
            case 2:
                if (par2 > 0) {
                    this.fabricator.setFluidAmount(par2);
                }
                else {
                    this.fabricator.clearFluid();
                }
                break;
            default:
        }
    ...
    public boolean canInteractWith(EntityPlayer par1EntityPlayer) {
        return this.fabricator.isUseableByPlayer(par1EntityPlayer);
    }
